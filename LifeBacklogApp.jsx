...
function Modal({ task, onSave, onClose }) {
  ...
  const inputStyle = { width: "100%", background: "#0d0d14", border: "1px solid #2d2d44", borderRadius: 6, padding: "9px 12px", color: "#e2e8f0", fontFamily: "inherit", fontSize: 12, outline: "none" };
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#13131f", border: "1px solid #2d2d44", borderRadius: 12, padding: 24, width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto", fontFamily: "'DM Mono', monospace" }}>
        ...
        {[["TITLE", "title", "text", "What needs to be decided or done?"], ["DEADLINE", "deadline", "text", "e.g. End of month, This Friday…"], ["CONTEXT", "details", "textarea", "Background, options, blockers…"], ["NEXT ACTION", "action", "textarea", "The very next concrete step…"]].map(([label, key, type, ph]) => (
          <div key={key} style={{ marginBottom: 14 }}>
            ...
            {type === "textarea"
              ? <textarea value={form[key]} onChange={e => set(key, e.target.value)} placeholder={ph} rows={3} style={{ ...inputStyle, resize: "vertical" }} />
...
export default function App() {
  ...
  const onDragEnd = () => {
    ...
    setDragId(null); dragOver.current = null;
  };
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, marginBottom: 28, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ fontSize: 10, color: "#4a5568", marginTop: 3 }}>{tasks.length} tasks · drag to reorder · data stays in browser</div>
          </div>
          <button onClick={() => setModal("new")} style={{ padding: "9px 18px", borderRadius: 8, background: "#e2e8f0", border: "none", color: "#0d0d14", fontSize: 10, fontWeight: 700, letterSpacing: 1.5, cursor: "pointer" }}>+ NEW TASK</button>
        </div>
        ...
        {PRIORITIES.map(p => {
          ...
          return (
            <div key={p.key} style={{ marginBottom: 28 }}>
              <div style={{ fontSize: 9, letterSpacing: 3, color: p.color, marginBottom: 8 }}>{p.label}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {group.map(task => {
                  const sc = STATUS_COLORS[task.status] || STATUS_COLORS["TO DO"];
                  const isOpen = expanded[task.id];
                  return (
                    <div key={task.id} className="tc" draggable onDragStart={() => setDragId(task.id)} onDragEnter={() => { dragOver.current = task.id; }} onDragEnd={onDragEnd} onDragOver={e => e.preventDefault()}
                      style={{ background: "#13131f", border: `1px solid ${isOpen ? p.border : "#1e1e2e"}`, borderLeft: `3px solid ${p.color}`, borderRadius: 8, padding: "11px 14px", opacity: dragId === task.id ? 0.4 : 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <span style={{ color: "#2d2d44", fontSize: 13, cursor: "grab" }}>⠿</span>
                        ...
                        <div className="ta" style={{ display: "flex", gap: 5, opacity: 0, transition: "opacity 0.15s" }}>
                          <button onClick={() => setModal(task)} style={{ background: "transparent", border: "1px solid #2d2d44", borderRadius: 4, padding: "2px 7px", color: "#718096", fontSize: 9, cursor: "pointer", fontFamily: "inherit" }}>edit</button>
                          <button onClick={() => deleteTask(task.id)} style={{ background: "transparent", border: "1px solid #3a1a1a", borderRadius: 4, padding: "2px 7px", color: "#ff4d4d", fontSize: 9, cursor: "pointer", fontFamily: "inherit" }}>del</button>
                        </div>
        ...
        {filtered.length === 0 && <div style={{ textAlign: "center", color: "#2d2d44", padding: "50px 0", fontSize: 12 }}>No tasks yet — add your first one ↑</div>}
